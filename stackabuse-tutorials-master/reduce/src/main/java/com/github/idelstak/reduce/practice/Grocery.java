...
public class Grocery {
  ...
  public static void main(String[] args) {
    ...
    Price totalPrice =
        transactions.stream().map(Transaction::getTotalPrice).reduce(Price.NIL, Price::add);

    System.out.printf("Total price: %s\n", totalPrice);
    // Total weight of items in the transaction
    Weight totalWeight =
        transactions.stream().map(Transaction::getTotalWeight).reduce(Weight.NIL, Weight::add);
